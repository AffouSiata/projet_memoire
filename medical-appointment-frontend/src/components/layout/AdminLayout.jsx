...
const AdminLayout = ({ children }) => {
  ...
  // Re-créer le menu quand la langue change
  const menuItems = useMemo(() => [
    ...
    {
      name: t('admin.nav.patients'),
      path: '/admin/patients',
      icon: UsersIcon,
      color: 'secondary-500',
    },
    {
      name: t('admin.nav.doctors'),
      path: '/admin/medecins',
      icon: UserGroupIcon,
      color: 'secondary-500',
    },
    {
      name: t('admin.nav.appointments'),
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-gray-900 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-secondary-400/20 dark:bg-secondary-500/10 rounded-full blur-3xl animate-blob animation-delay-2000"></div>
      ...
      >
        ...
        <div className="relative flex items-center justify-between p-6 border-b border-slate-200/50 dark:border-gray-700/50">
          {isSidebarOpen && (
            <div className="flex items-center gap-3 animate-slide-in-left">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-white font-medium">{t('layout.adminSpace')}</p>
              </div>
            </div>
          )}
          <button
        ...
        {/* User Info */}
        {isSidebarOpen && user && (
          <div className="relative p-6 border-b border-slate-200/50 dark:border-gray-700/50 animate-slide-in-left" style={{ animationDelay: '100ms' }}>
            <div className="relative group w-full">
              <div className="relative flex items-center gap-4 p-4 bg-white/60 dark:bg-gray-700/60 rounded-2xl backdrop-blur-sm border border-white/50 dark:border-gray-600/50 shadow-lg">
                <div className="relative">
                  <div className="relative w-14 h-14 bg-gradient-to-br from-secondary-500 to-emerald-600 rounded-2xl flex items-center justify-center text-white shadow-xl">
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  <p className="text-xs text-secondary-600 dark:text-secondary-400 truncate font-medium">{t('layout.administrator')}</p>
                  <div className="flex items-center gap-1 mt-1">
                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                    <span className="text-xs text-green-600 font-medium">{t('layout.online')}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Navigation */}
        <nav className="relative p-4 space-y-2 overflow-y-auto scrollbar-hide" style={{ maxHeight: 'calc(100vh - 340px)' }}>
        ...
        {/* Logout Button */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-200/50 dark:border-gray-700/50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
          <button
            onClick={handleLogout}
            className="relative w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-all duration-300 group hover:scale-105 hover:shadow-lg"
          >
            <ArrowLeftOnRectangleIcon className="relative w-6 h-6 group-hover:scale-110 transition-transform" />
            {isSidebarOpen && (
              <span className="relative text-sm font-semibold">{t('common.logout')}</span>
            )}
          </button>
        </div>
      </aside>
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-4">
                ...
                >
                  ...
                  <span>{t('common.logoutButton')}</span>
                </button>
              </div>
            </div>
          </div>
