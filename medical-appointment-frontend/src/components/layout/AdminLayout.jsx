        ...
        {user && (
          <div className={`relative p-6 border-b border-slate-200/50 dark:border-gray-700/50 animate-slide-in-left ${!isSidebarOpen && 'md:p-4'}`} style={{ animationDelay: '100ms' }}>
            <div className="relative group w-full">
              <div className={`relative flex items-center gap-4 p-4 bg-white/60 dark:bg-gray-700/60 rounded-2xl backdrop-blur-sm border border-white/50 dark:border-gray-600/50 shadow-lg ${!isSidebarOpen && 'md:p-2 md:justify-center'}`}>
                <div className="relative">
                  <div className={`relative bg-blue-700 rounded-2xl flex items-center justify-center text-white shadow-xl ${isSidebarOpen ? 'w-14 h-14' : 'w-14 h-14 md:w-10 md:h-10'}`}>
                    <ShieldCheckIcon className={`${isSidebarOpen ? 'w-8 h-8' : 'w-8 h-8 md:w-6 md:h-6'}`} />
                  </div>
                </div>
