...
const MedecinNotes = () => {
  ...
  // Traduire le contenu des notes quand la langue change
  useEffect(() => {
    const translateNotes = async () => {
      const currentLang = i18n.language;

      // Si la langue est le français, pas besoin de traduire (langue originale)
      if (currentLang === 'fr') {
        setTranslatedContents({});
        return;
      }

      // Traduire le contenu de toutes les notes
      const translations = {};
      for (const note of notes) {
        if (note.contenu) {
          try {
            translations[note.id] = await translateText(note.contenu, currentLang, 'fr');
          } catch (error) {
            console.error('Error translating note:', error);
            translations[note.id] = note.contenu; // Garder l'original en cas d'erreur
          }
        }
      }
  ...
  // Fonction helper pour obtenir le contenu traduit ou original
  const getTranslatedContent = (note) => {
    // Si on a une traduction pour cette note, l'utiliser
    if (translatedContents[note.id]) {
      return translatedContents[note.id];
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (!formData.patientId || !formData.contenu.trim()) {
        setCreateError('Veuillez remplir tous les champs obligatoires');
        return;
      }

      // Ne pas envoyer le champ "statut" - il est géré automatiquement par le backend
      const { statut, ...noteData } = formData;
      await medecinService.createNote(noteData);
      loadData();
      ...
      const errorMessage = getErrorMessage(error, 'Une erreur est survenue lors de la création de la note');
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-500 dark:text-gray-400">
                                {formatDate(new Date(note.createdAt), dateFormats.medium)} • {t('common.dr')}. {user?.nom}
                              </p>
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 max-w-2xl w-full animate-scale-in overflow-hidden">
            ...
            <form onSubmit={handleCreate} className="relative z-10 space-y-6">
              ...
              {/* Message d'erreur */}
              {createError && (
                <div className="p-4 bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-2xl animate-shake">
